...
volcano_plot <- function(seurat_object,
  ...
  markers <- markers %>%
    dplyr::mutate(p_val_adj = ifelse(p_val_adj == 0e+00, 1e-299, p_val_adj))
  # markers$p_val_adj <-
  ...
  p.volcano <-  
    ...
    theme(
      ...
      legend.position = "bottom", #c(0.89, 0.55),
  ...
  if(plot_top == TRUE) {
    ...
    t2 <- 
      c(
        t %>% arrange(desc(avg_log2FC)) %>% .[1:n_genes, 6],
        t %>% arrange(avg_log2FC) %>% .[1:n_genes, 6]
      )
...
volcano_plot_in_cluster <- function(markers = NULL, 
  ...
  markers <- 
    markers %>% 
    dplyr::mutate(p_val_adj = ifelse(p_val_adj == 0e+00, runif(n_zeros, max = 10^-280, min = 10^-289), p_val_adj)) %>%
  ...
  p.volcano_cluster <-
    ggplot() +
    geom_point(data = p.volcano_data,
               mapping = 
                 aes(x = xjitter, 
                     y = avg_log2FC,
                     color = sig,
    ...
    geom_label_repel(data = p.volcano_data %>% 
                       group_by(cluster) %>% 
                       filter(avg_log2FC > 0) %>%
                       {
                         if(order.by == "avg_log2FC") {
                           top_n(., n = n_pos, wt = avg_log2FC)
                         } else {
                           top_n(., n = n_neg, wt = p_val_adj)
                         }
                       }, 
